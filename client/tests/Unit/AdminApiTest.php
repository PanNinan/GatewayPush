...
final class AdminApiTest extends TestCase
{
    ...
    public const API_SECRET = 'api-secret-0123456789abcdef';
    ...
    {
        $api = $this->makeApi($fake);
        $api->push('uid', 'alice', ['title' => 'hi'], ['msg_id' => 'm-1', 'offline_mode' => 'queue'], function () {});
        ...
        self::assertSame(['title' => 'hi'], $job['payload']);
    ...
    public function testStatsSuccessTriplet()
    {
        ...
        $snapshot = ['gauge' => ['conn_total' => 3], 'counter' => ['msg_in' => 10]];
        $this->respond(200, json_encode(['code' => 0, 'msg' => 'ok', 'ts' => time(), 'data' => $snapshot]));
    ...
    public function testBadSignResponds401Triplet()
    {
        ...
        $this->respond(401, json_encode(['code' => 4001, 'msg' => '签名校验失败', 'ts' => time()]));
    ...
    public function testBusinessCodeNonZeroFailsEvenOn200()
    {
        ...
        $this->respond(200, json_encode(['code' => 4000, 'msg' => 'target_type 必须是 uid / device / client 之一']));
    ...
    public function testEmptySecretStillSendsRequest()
    {
        ...
        $this->respond(500, json_encode(['code' => 5000, 'msg' => '服务端未配置接口密钥']));
        self::assertFalse($out[0]);
        self::assertSame(ErrorCode::HTTP_SERVER_ERROR, $out[2]['code']);
    }

...
class ActionReplyTest extends TestCase
{
    ...
    public function testNonHttpClientIsRejected(): void
    {
        ...
        foreach (array('7', '123456789', 'udp:127.0.0.1:53210', '') as $clientId) {
    ...
    {
        return array(
            '十六进制（生产形态）' => array('9f2c1a4b7e6d5c30', true),
            '含连字符与下划线'     => array('req-1_a', true),
            '单字符'               => array('a', true),
            '64 字符（上限）'      => array(str_repeat('a', 64), true),
            '65 字符（越界）'      => array(str_repeat('a', 65), false),
            '空串'                 => array('', false),
            // 路径穿越是键空间污染的主要形态，必须拦住
            '路径穿越 ../'         => array('../evil', false),
            '含冒号'               => array('a:b', false),
            '含空格'               => array('a b', false),
            '含换行'               => array("a\nb", false),
            '含中文'               => array('请求', false),
    ...
    {
        ActionReply::init(array('result_ttl' => $configured));
        $this->assertSame($expected, ActionReply::ttl());

        // 还原默认值，避免影响同进程内的后续用例
        ActionReply::init(array('result_ttl' => ActionReply::DEFAULT_TTL));
    }
    ...
    {
        ActionReply::init(array('result_ttl' => 90));
        ActionReply::init(array('enable' => true));

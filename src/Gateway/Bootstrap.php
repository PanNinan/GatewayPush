...
class Bootstrap
{
    ...
    public static function init(array $gatewayConfig, array $appConfig): void
    {
        ...
        UdpProtocol::$maxPacketSize = (int)($gatewayConfig['udp']['max_packet_size'] ?? 8192);
    ...
    public static function onUdpMessage(ConnectionInterface $connection, $packet): void
    {
        try {
            ...
            if (!RateLimiter::checkMemory(RateLimiter::DIM_IP, $ip)) {
                ...
                RateLimiter::logReject(RateLimiter::DIM_IP, $ip, [
                    'channel'   => 'udp-gateway',
            ...
            $queueConf = self::$config['udp']['queue'] ?? [];
    ...
    public static function consumeUdpOutQueue($worker, array $conf): void
    {
        try {
            $key   = $conf['key'];
            $batch = max(1, (int)($conf['batch'] ?? 200));
            ...
            RedisClient::popBatch($key, $batch, function ($items) use ($worker) {
                foreach ($items as $raw) {
                    ...
                    if (!is_array($task) || empty($task['client_id']) || empty($task['frame'])) {
                        ...
                        Logger::warn('UDP 出站任务格式非法，已丢弃', ['raw' => substr((string)$raw, 0, 200)]);
    ...
    protected static function initRegister(): void
    {
        ...
        WorkerEvents::bind($register, $conf['name'], ['buffer' => false]);
    }
    ...
    protected static function initWebSocketGateway(): void
    {
        ...
        $gateway->onBusinessWorkerConnected = function ($connection) {
            Logger::info('BusinessWorker 已接入网关', [
                'worker_key' => $connection->key ?? '',
            ]);
        };

        $gateway->onBusinessWorkerClose = function ($connection) {
            Logger::warn('BusinessWorker 与网关连接断开', [
                'worker_key' => $connection->key ?? '',
            ]);
        };
    ...
    protected static function initUdpGateway(): void
    {
        ...
        $udp->onMessage = [self::class, 'onUdpMessage'];
        ...
        $udp->onWorkerStart = function ($worker) use ($conf) {
            ...
            RateLimiter::init(self::$appConfig['rate_limit'] ?? []);
        ...
        $udp->onWorkerStop = function ($worker) {
            Logger::info('UDP 网关正在停止，释放连接资源', ['id' => $worker->id]);
            RedisClient::closeAll();
        };

        // UDP 为无连接协议，不存在 TCP 发送缓冲背压，仅绑错误事件
        WorkerEvents::bind($udp, $conf['name'], ['buffer' => false]);
    }
    ...
    protected static function pushToBusinessQueue(ConnectionInterface $connection, array $packet, array $queueConf): void
    {
        ...
        if ($job === false) {
            Logger::error('UDP 队列任务序列化失败', ['cmd' => $packet['cmd']]);

            return;
        }

        $maxLen = (int)($queueConf['max_len'] ?? 0);
        ...
        RedisClient::rPush($queueKey, $job, function ($result) use ($maxLen, $queueKey) {
            ...
            if ($maxLen > 0 && $result > $maxLen) {
                Logger::warn('UDP 队列积压超限，业务消费能力不足', [
                    'queue'  => $queueKey,
                    'length' => $result,
                    'max'    => $maxLen,
                ]);
    ...
    protected static function sendUdp($worker, string $clientId, string $frame, array $task = []): void
    {
        ...
        if ($address === '') {
            Monitor::incr('udp_out_fail');
            Logger::warn('UDP 出站目标地址无法解析', ['client_id' => $clientId]);
        ...
        if (!$socket) {
            Monitor::incr('udp_out_fail');
            Logger::error('UDP 网关 socket 不可用，出站报文丢弃', ['client_id' => $clientId]);
